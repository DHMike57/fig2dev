From dhliu@solar.csie.ntu.edu.tw Mon Mar 16 09:08:41 1992
Return-Path: <dhliu@solar.csie.ntu.edu.tw>
Received: by epb7.lbl.gov (5.57/SMI-3.2)
	id AA00470; Mon, 16 Mar 92 09:08:04 -0800
Received: from epb2.lbl.gov by lbl.gov (4.1/1.39)
	id AA17555; Mon, 16 Mar 92 04:22:27 PST
Received: by epb2.lbl.gov (5.57/SMI-3.2)
	id AA06812; Mon, 16 Mar 92 04:22:43 PST
Received: from csman ([140.112.30.21]) by lbl.gov (4.1/1.39)
	id AA17551; Mon, 16 Mar 92 04:21:54 PST
Received: from solar ([140.112.28.160]) by csman (4.1/SMI-4.1)
	id AA00605; Mon, 16 Mar 92 20:22:44 CST
Received: from uranus.syslab ([140.112.28.167]) by solar (4.1/SMI-4.1)
	id AA00959; Mon, 16 Mar 92 20:24:34 CST
Date: Mon, 16 Mar 92 20:24:34 CST
From: dhliu@solar.csie.ntu.edu.tw (Der-Hua Liu)
Message-Id: <9203161224.AA00959@solar>
Apparently-To: <bvsmith@lbl.gov>
Status: OR
...
F_line	*l;
{
	...
	if (l->back_arrow && l->thickness > 0)
	    draw_arrow_hea2((double)q->x, (double)q->y, (double)p->x,
...
ugh tr to tl */
	...
	if (l->for_arrow && l->thickness > 0)
	    draw_arrow_hea2((double)p->x, (double)p->y, (double)q->x,
...
F_spline	*s;
{
	...
	p = s->points;
	    ...
	    draw_arrow_hea2(a->rx, a->ry, (double)p->x,
	...
	if (s->for_arrow && s->thickness > 0)
	    draw_arrow_hea2(a->lx, a->ly, (double)p->x,
...
F_spline	*s;
{
	...
	*/
	...
	y3 = b = (y1 + d) / 2;
	if (s->back_arrow && s->thickness > 0) {
	    draw_arrow_hea2(c, d, x1, y1, s->back_arrow->ht, s->back_arrow->wid,
			    s->color);
	    }
	set_style(s->style, s->style_val);
	if (! closed_spline(s)) {
	...
	*/
	...
	reset_style(s->style, s->style_val);
	if (s->for_arrow && s->thickness > 0) {
	    draw_arrow_hea2(x2, y2, c, d, s->for_arrow->ht,
...
static draw_arrow_hea2(x1, y1, x2, y2, arrowht, arrowwid, col)
double	x1, y1, x2, y2, arrowht, arrowwid;
int col;
{
	double	x, y, xb, yb, dx, dy, l, sina, cosa;
	...
	yd = -x*sina + y*cosa;
        fprintf(tfp, "%% Polyline\n");
        fprintf(tfp, "n %.3f %.3f m %.3f %.3f l %.3f %.3f l  clp gs 0.00 setgray fill gr \ngs col%d s gr\n",xc, yc, x2, y2, xd, yd, col > MAXCOLORS ? -1 : col);
}
